...
class TruckAnomaliesSuite extends FunSuite {
  ...
  test("test simple anomaly filter") {
    val threshold=10
    assert(true===TrucksFilter.shouldSend(true,threshold,None,1,40))
    assert(true===TrucksFilter.shouldSend(false,threshold,None,1,40))
    assert(true===TrucksFilter.shouldSend(true, threshold,None,21,20))
    assert(true===TrucksFilter.shouldSend(false, threshold,None,21,20))
